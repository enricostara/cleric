...
static void validator_error(ValidatorContext *ctx, AstNode *node, const char *format, ...) {
    ...
    if (node && node->type == NODE_VAR_DECL) {
        // Example: use token from VarDeclNode
        Token t = ((VarDeclNode *) node)->declaration_token;
        fprintf(stderr, "Validation Error (position %zu, near '%s'): %s\n", t.position, t.lexeme, buffer);
    } else if (node && node->type == NODE_IDENTIFIER) {
        // Example: use token info if available (not yet in IdentifierNode)
...
static bool validate_block_node(const BlockNode *node, ValidatorContext *ctx) {
    ...
    for (size_t i = 0; i < node->num_items; ++i) {
        ...
        if (ctx->error_flag) {
            // Stop processing items in block if an error occurred
            symbol_table_exit_scope(ctx->st);
            return false;
        }
    }
