...
int main(int argc, char *argv[]) {
    ...
    const char *input_file = parse_args(argc, argv, &lex_only, &parse_only, &tac_only, &codegen_only);
    ...
    if (run_compiler(i_file, lex_only, parse_only, tac_only, codegen_only) != 0) return 1;

    // Skip assembly/linking if any "only" mode is active
    if (!lex_only && !parse_only && !tac_only && !codegen_only) {
        char s_file[1024];
