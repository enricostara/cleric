...
static void verify_var_decl_node(AstNode *node, const char *expected_type_name, const char *expected_var_name,
                                 bool expect_initializer, NodeType expected_initializer_type_if_any) {
    ...
    if (expect_initializer) {
        ...
        TEST_ASSERT_EQUAL_MESSAGE(expected_initializer_type_if_any, decl_node->initializer->type,
                                  "Initializer node type mismatch");
...
static void verify_assignment_node(AstNode *node, const char *expected_left_var_name,
                                   NodeType expected_right_node_type) {
    ...
    TEST_ASSERT_EQUAL_STRING_MESSAGE(expected_left_var_name, left_ident->name,
                                     "Assignment target variable name mismatch.");

    // Verify right side (value) type
    TEST_ASSERT_NOT_NULL_MESSAGE(assign_node->value, "Assignment value (right side) is NULL");
    TEST_ASSERT_EQUAL_MESSAGE(expected_right_node_type, assign_node->value->type,
                              "Assignment value node type mismatch.");
}
...
void test_parse_simple_declaration_with_initializer(void) {
    ...
    TEST_ASSERT_FALSE_MESSAGE(parser.error_flag,
                              parser.error_message ? parser.error_message : "Parser error flag set.");
    ...
    TEST_ASSERT_GREATER_OR_EQUAL_INT_MESSAGE(2, body_block->num_items,
                                             "Block should have at least 2 items (declaration and return)");
...
void test_parse_simple_assignment_statement(void) {
    ...
    TEST_ASSERT_GREATER_OR_EQUAL_INT_MESSAGE(3, body_block->num_items,
                                             "Block should have at least 3 items (declaration, assignment, return)");
