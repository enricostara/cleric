...
static void setup_parser_for_test(const char *source, Lexer *lexer, Parser *parser, Arena *ast_arena,
                                  Arena *lexer_arena) {
...
void test_parse_declaration_and_return__int_var_then_return_zero(void) {
    ...
    TEST_ASSERT_EQUAL_UINT_MESSAGE(NODE_INT_LITERAL, ret_stmt->expression->type,
                                   "Return expression should be IntLiteralNode.");
...
void test_parse_expression_statement__literal_expression(void) {
    ...
    TEST_ASSERT_EQUAL_UINT_MESSAGE(NODE_INT_LITERAL, item0_expr_stmt->type,
                                   "Item 0 (expression statement) should be IntLiteralNode.");
...
void test_parse_nested_block__inner_block_with_declaration(void) {
    ...
    TEST_ASSERT_EQUAL_UINT_MESSAGE(NODE_BLOCK, item0_nested_block_node->type,
                                   "Item 0 should be BlockNode (nested block).");
    ...
    TEST_ASSERT_EQUAL_UINT_MESSAGE(NODE_VAR_DECL, inner_item0_var_decl->type,
                                   "Inner block item 0 should be VarDeclNode.");
